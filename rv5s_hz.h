...
public:
    ...
    RV5S_HZ(const QStringList& extensions) : RipesProcessor("5-Stage RISC-V Processor with hazard detection") {
        ...
        exmem_reg->wr_reg_idx_out >> hzunit->mem_reg_wr_idx; // the index of write register of mem stage
        memwb_reg->wr_reg_idx_out >> hzunit->wb_reg_wr_idx; // the index of write register of wb stage
        
        idex_reg->reg_do_write_out >> hzunit->ex_do_reg_write; // whether the current instruction at EX stage will write registers.
